...
- (UIImage *)createImage
{
    ...
    // 首先绘制路径数组中的所有路径
    for (XHVPathOfDraw *drawPath in self.pathsArray) {
        // 此时，路径中多了图像的可能
        // 判断是否是图像
        // 如果是图像，绘制图像
        // 如果不是图像，就是路径，直接绘制
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    XHVPathOfDraw *path = [XHVPathOfDraw drawPathWith:self.path color:self.color lineWidth:self.lineWidth];
...
- (void)setImage:(UIImage *)image
{
    ...
    XHVPathOfDraw *path = [[XHVPathOfDraw alloc] init];
    path.image = image;
